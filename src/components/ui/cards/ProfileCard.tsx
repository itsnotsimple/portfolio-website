...
const ProfileCardComponent: React.FC<ProfileCardProps> = ({
  ...
  const tiltEngine = useMemo(() => {
    ...
    const setVarsFromXY = (x: number, y: number) => {
      ...
      const properties: Record<string, string> = {
        ...
        '--pc-rotate-x': `${round(-(centerY / 5))}deg`,
        '--pc-rotate-y': `${round(centerX / 4)}deg`,
      };
    ...
    return {
      ...
      toCenter() {
        const shell = shellRef.current;
        if (!shell) return;
        this.setTarget(shell.clientWidth * 0.02, shell.clientHeight * 0.20);
      },
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      ...
      const DeviceMotionEventIOS = window.DeviceMotionEvent as unknown as {
        requestPermission?: () => Promise<PermissionState>;
      };
      if (typeof DeviceMotionEventIOS?.requestPermission === 'function') {
        DeviceMotionEventIOS.requestPermission!().then((state: string) => {
                ...
                <div className="pc-user-info">
                  <div className="pc-user-details">
                    <div className="pc-mini-avatar">
                      <img src={miniAvatarUrl || avatarUrl} alt={`${name} mini`} width={44} height={44} loading="lazy" decoding="async" />
                    </div>
